...
def main():
    ...
    # Create file uploader for data input
    uploaded_file = st.file_uploader('Upload your wind data (CSV)', type=['csv', 'txt'])

    if uploaded_file is not None:
        df = pd.read_csv(uploaded_file, index_col='Date_Time', sep='\t')
        ...
        T6_gen = np.hstack(
            [diurnal_fourier_gen, seasonal_fourier_gen, interaction(diurnal_fourier_gen, seasonal_fourier_gen)])
        ...
        # Example: displaying the first few rows of the resulting DataFrame
